...
import { Search, X, ShoppingBag, Layers, ChevronRight, Loader2 } from 'lucide-react';

export const SearchOverlay: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const [query, setQuery] = useState('');
    const [results, setResults] = useState<{ products: any[], collections: any[] }>({ products: [], collections: [] });
    ...
    const executeSearch = async () => {
        ...
        setResults({ products: products || [], collections: collections || [] });
        setLoading(false);
    };
