...
import { Layers, Plus, Filter, ChevronRight } from 'lucide-react';
...
const CollectionTypeCard = ({ type, label, active, onSelect }: { type: string; label: string; active: boolean; onSelect: (type: string) => void }) => (
    <button 
        type="button"
...
export const CollectionManager: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => fetchCollections(), 0);
            ...
            {isCreating && (
                <div style={{ background: '#131316', border: '1px solid #27272a', borderRadius: 12, padding: 32, marginBottom: 32 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32 }}>
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: 16 }}>
                                <CollectionTypeCard type="manual" label="Manual" active={newCollection.type === 'manual'} onSelect={(t) => setNewCollection({...newCollection, type: t})} />
                                <CollectionTypeCard type="smart" label="Automated (Smart)" active={newCollection.type === 'smart'} onSelect={(t) => setNewCollection({...newCollection, type: t})} />
                            </div>
                            ...
                            {newCollection.type === 'smart' && (
                                <div style={{ marginTop: 24, padding: 20, background: '#09090b', border: '1px dashed #27272a', borderRadius: 12 }}>
                                    ...
                                    <button type="button" style={{ marginTop: 12, fontSize: 12, color: '#FF6B35', background: 'transparent', border: 'none', fontWeight: 600, cursor: 'pointer' }}>+ Add Condition</button>
                                </div>
                            )}
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: 12, marginTop: 40, justifyContent: 'flex-end', borderTop: '1px solid #27272a', paddingTop: 24 }}>
                        ...
                        <button type="button" onClick={handleCreate} style={{ padding: '10px 24px', background: '#FF6B35', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600, cursor: 'pointer' }}>Save Collection</button>
                    </div>
                </div>
            )}
                ...
                {loading ? (
                    ...
                    <div key={c.id} style={{ background: '#131316', border: '1px solid #27272a', borderRadius: 16, overflow: 'hidden', cursor: 'pointer' }}>
                        <div style={{ height: 120, background: '#1c1c22', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {c.image_url ? <img src={c.image_url} alt={c.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Layers size={40} color="#27272a" />}
                        </div>
