...
class SymbolManager {
     ...
     */
    createSymbol(name: string, node: { type: string; props: Record<string, unknown>; styles: Record<string, unknown>; responsive?: Record<string, unknown>; children: string[] }): string {
    ...
    propagateIfSymbol(
        ...
        const symbolId = (sourceNode as Record<string, unknown>).symbolId as string | undefined;
        if (!symbolId) return [];

        // 1. Update master blueprint
        const blueprint = this.registry.get(symbolId);
        if (blueprint) {
            this.setByPath(blueprint as unknown as Record<string, unknown>, patchPath, patchValue);
        ...
        for (const id of Object.keys(allNodes)) {
            ...
            if ((allNodes[id] as Record<string, unknown>).symbolId === symbolId) {
                siblingIds.push(id);
            }
        }
