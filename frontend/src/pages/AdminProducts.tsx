    ...
    return (
        <div className="admin-products animate-fade-in">
            <div className="page-header">
                ...
                <button type="button" className="add-btn" onClick={() => { setEditingId(null); setFormData(INITIAL_FORM_STATE); setIsModalOpen(true); }}>
                    ...
                    <tbody>
                        {products.map(product => (
                            <tr key={product._id}>
                                ...
                                <td className="price-cell">PKR {(product.price || 0).toLocaleString()}</td>
                                <td>
                ...
                <div className="modal-overlay">
                    <div className="modal-content animate-scale-up">
                        <div className="modal-header">
                            ...
                            <button type="button" className="close-modal" onClick={resetForm} aria-label="Close modal"><X size={20} /></button>
                        </div>
                        <form className="product-form" onSubmit={handleSubmit}>
                            <div className="form-group">
                                <label htmlFor="productName">ASSET NAME</label>
                                <input id="productName" name="name" value={formData.name} onChange={handleChange} required className="glass-input" />
                            </div>

                            <div className="form-group">
                                <label htmlFor="productDesc">DESCRIPTION</label>
                                <textarea id="productDesc" name="description" value={formData.description} onChange={handleChange} required rows={3} className="glass-input" />
                            </div>

                            <div className="form-row">
                                <div className="form-group">
                                    <label htmlFor="productPrice">VALUE (PKR)</label>
                                    <input id="productPrice" type="number" name="price" value={formData.price} onChange={handleChange} required min={0} className="glass-input" />
                                </div>
                                <div className="form-group">
                                    <label htmlFor="productStock">STOCK LEVEL</label>
                                    <input id="productStock" type="number" name="stock" value={formData.stock} onChange={handleChange} required min={0} className="glass-input" />
                                </div>
                            </div>

                            <div className="form-group">
                                <label htmlFor="productCategory">CATEGORY_ID</label>
                                <input id="productCategory" name="category" value={formData.category} onChange={handleChange} required className="glass-input" />
                            </div>

                            <div className="form-group">
                                <label htmlFor="productImage">IMAGE SOURCE</label>
                                <input id="productImage" name="image" value={formData.image} onChange={handleChange} required className="glass-input" />
                            </div>
