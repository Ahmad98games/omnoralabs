...
class InventoryController {
  ...
  static async getInventoryStatus(req, res) {
    try {
      ...
      const stock = await InventoryService.getAvailableStock(mongoProductId);
      let availability = 'Out of Stock';
      if (stock > 10) availability = 'In Stock';
      else if (stock > 0) availability = 'Low Stock';
  ...
  static async checkStock(req, res) {
    try {
      ...
      return res.json({
        success: true,
        inStock: available > 0,
        availability: available > 10 ? 'In Stock' : (available > 0 ? 'Low Stock' : 'Out of Stock')
      });
    } catch (error) {
