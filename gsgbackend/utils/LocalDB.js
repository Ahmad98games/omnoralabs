...
class LocalDB {
    constructor(collectionName, schema = {}, tenantId = null) {
        ...
        // Load data
        this._load();
        console.log(`[LocalDB] Loaded ${this.collectionName} from ${this.filePath} - Count: ${this.data.length}`);
    }
    ...
    find(query = {}) {
        const scopedQuery = this.tenantId ? { ...query, tenant_id: this.tenantId } : query;
        let results = this.data.filter(item => this._matches(item, scopedQuery));
    ...
    findOne(query) {
        const scopedQuery = this.tenantId ? { ...query, tenant_id: this.tenantId } : query;
        const doc = this.data.find(item => this._matches(item, scopedQuery));
        return this._chain(doc ? this._wrapDocument(doc) : null);
    }
    ...
    _matches(item, query) {
        ...
        const activeTenantId = this.tenantId || (store && store.tenantId);

        if (query.tenant_id) {
            console.log(`[_matches] item.name: ${item.name} - activeTenantId: ${activeTenantId} - query.tenant_id: ${query.tenant_id}`);
        }

        // Scope validation: 
        // 1. If activeTenantId is present and item has a DIFFERENT tenant_id, skip.
        // 2. If item has NO tenant_id, we allow it (for migration/backwards compatibility).
        if (activeTenantId && item.tenant_id && item.tenant_id !== activeTenantId) {
            return false;
        }
        ...
        for (const [key, value] of Object.entries(query)) {
            ...
            // Allow missing tenant_id to match 'default_tenant'
            if (key === 'tenant_id' && value === 'default_tenant' && !item[key]) {
                continue;
            }
